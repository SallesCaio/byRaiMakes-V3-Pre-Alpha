...
export class AdminDashboardPage implements OnInit, OnDestroy {
  ...
  carregarStats() {
    ...
    this.firestore.doc(`caixa/${hoje}`).valueChanges()
      .subscribe((c: any) => this.caixaHoje = c?.total || 0);
  }

  async confirmarVenda(id: string, valor: number) {
    try {
      // import dinâmico para não acoplar imports circulares
      const { PedidoService } = await import('../../services/pedido.service');
      const svc = new PedidoService(this.firestore, this.afAuth);
      await svc.confirmarVenda(id, valor);
